...
def get_connection_url_from_credentials_file(credential_path: os.path) -> URL:
    ...
    return URL.create(
        ...
        port=credentials["host_port_to_database"],
    )


def get_engine_from_credentials_file(
    credential_path: os.path, use_sqlalchemy_v2: bool = False, echo: bool = False
) -> Engine:
    connection_url = get_connection_url_from_credentials_file(credential_path=credential_path)
    engine = create_engine(connection_url, future=use_sqlalchemy_v2, echo=echo)
    return engine
